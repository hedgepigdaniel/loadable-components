...
const loadablePlugin = api => {
  ...
  function transformImport(path) {
    const importCreator = path.node.type === 'CallExpression' ? path.node.arguments[0] // loadable((...) => import(...)) or loadable.lib
    : path.node; // /* #__LOADABLE__ */ () => import(...)

    if (!isFunctionAndOnlyReturnsImport(importCreator)) {
      throw new Error('The first argument to `loadable()` must be a function with a single statement that returns a call to `import()`');
    }

    const callPaths = collectImportCallPaths(path); // Multiple imports call is not supported
